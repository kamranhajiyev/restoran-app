...
export default function SellerPage({ overrideCompanyId, overrideCompanyName }: { overrideCompanyId?: string; overrideCompanyName?: string } = {}) {
  ...
  useEffect(() => {
    if (overrideCompanyId) {
      ...
      companyIdRef.current = overrideCompanyId;
    ...
    companyIdRef.current = session.companyId;
        ...
        {!collapsed ? (
          <div className="px-4 py-4 border-t border-stone-100/50">
            <div className="flex items-center gap-2 mb-3">
              ...
              {!online && (
                <span className="ml-auto text-[10px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full flex items-center gap-1">
                  Oflayn
                  {queueCount > 0 && (
                    <span className="bg-amber-700 text-white rounded-full px-1 min-w-[14px] text-center leading-none">{queueCount}</span>
                  )}
                </span>
              )}
            </div>
