...
export default function SellerPage({ overrideCompanyId, overrideCompanyName, overrideToken, overrideLogoUrl, overrideBrandColor }: { overrideCompanyId?: string; overrideCompanyName?: string; overrideToken?: string; overrideLogoUrl?: string | null; overrideBrandColor?: string | null } = {}) {
  ...
  const ownOrders = useRef<Set<string>>(new Set());
  const ownRemovedItems = useRef<Set<string>>(new Set());
  // Appends can't be suppressed by item id: order_items ids are generated by the
  // server, so this device can't know them in advance. Count our own appends per
  // order instead and let the diff consume one marker per detected append.
  const ownAppends = useRef<Map<string, number>>(new Map());
  // Item ids per order id. Null until the first list arrives, so loading 40
  // existing orders on startup doesn't fire 40 chimes.
  const seen = useRef<Map<string, Set<string>> | null>(null);
  ...
  useEffect(() => {
    const snapshot = new Map<string, Set<string>>(
      orders.map(o => [o.id, new Set(o.items.map(i => i.id).filter((x): x is string => !!x))]),
    );
    ...
    let newWork = [...snapshot.keys()].some(id => !prev.has(id) && !ownOrders.current.has(id));
    let removed = false;

    for (const [orderId, before] of prev) {
      const now = snapshot.get(orderId);
      // An order missing from the new list fell off the end of the 200-row window
      // as newer ones arrived. That is not a removal.
      if (!now) continue;

      // Items added to an already-open order are new work for the kitchen too —
      // same chime as a new order, or a waiter's "one more kebab" goes unheard.
      if ([...now].some(id => !before.has(id))) {
        const pending = ownAppends.current.get(orderId) ?? 0;
        if (pending > 0) ownAppends.current.set(orderId, pending - 1);
        else newWork = true;
      }

      if ([...before].some(id => !now.has(id) && !ownRemovedItems.current.has(id))) removed = true;
    }

    if (newWork) playNewOrder();
    else if (removed) playItemRemoved();      // a new order is the louder signal; don't stack both
  ...
  async function submitAppend() {
    ...
    setSubmitting(true);
    ownAppends.current.set(orderId, (ownAppends.current.get(orderId) ?? 0) + 1);   // our own append — don't alert ourselves
    ...
    setSubmitting(false);
    if (saveError) {
      // Nothing was appended, so drop the marker — otherwise it would swallow the
      // chime for the next genuine append from another device.
      const n = (ownAppends.current.get(orderId) ?? 1) - 1;
      if (n > 0) ownAppends.current.set(orderId, n); else ownAppends.current.delete(orderId);
  ...
  async function handleDecrementItem(order: Order, oi: OrderItem) {
    ...
    if (newQty <= 0) ownRemovedItems.current.add(oi.id);   // our own removal — don't alert ourselves
    ...
    if (!ok) { ownRemovedItems.current.delete(oi.id); alert('Dəyişdirilmədi. Yenidən cəhd edin.'); return; }
  ...
  async function submitOrder() {
    ...
    ownOrders.current.add(order.id);   // our own order — the other devices beep, not this one
